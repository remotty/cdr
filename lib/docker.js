...
docker = (function(){
  var remote = process.env.DOCKER_REMOTE;
  
  if(remote === 'https'){
    return new Docker({
      protocol: process.env.DOCKER_PROTOCOL || 'https', // 'https'
  ...
  }else if(remote === 'tcp' || remote === 'http'){
    return new Docker({
      protocol: process.env.DOCKER_PROTOCOL || 'tcp', // 'https'
      host: process.env.DOCKER_HOST || '172.17.42.1',
      port: process.env.DOCKER_PORT || 2376,
    });
  }else{
...
module.exports.build = function build(name, image, cb)
{
    var target = "./docker/" + name + ".docker",
        source = "./Dockerfile";
    console.log('Copying ' + target + ' to ' + source + ' for build');
